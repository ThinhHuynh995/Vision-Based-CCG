...
# 1. Tự động hoàn toàn — CLIP zero-shot (chưa có data gì)
python main.py autolabel --input data/raw/cctv.mp4

# 2. Tự động — model đã train (semi-supervised)
python main.py autolabel --input data/raw/cctv.mp4 --mode-al semi

# 3. AI-assisted label tool — AI gợi ý, người xác nhận
python main.py label --video data/raw/cctv.mp4 --ai

### Cách hoạt động

**Zero-shot CLIP** — không cần train gì trước:
Frame → detect person → crop → CLIP encode →
so sánh cosine similarity với text prompts:
  "a person fighting" / "a person falling" / ...
→ class có similarity cao nhất = nhãn


# AI gợi ý, bạn xác nhận
python main.py label --video data/raw/cctv.mp4 --ai

# ENTER  → chấp nhận nhãn AI
# 0–4    → ghi đè nhãn và lưu luôn
# Z      → undo frame vừa lưu
# D/A    → next/prev frame
